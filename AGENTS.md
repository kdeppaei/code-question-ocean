...
- A new problem must change the learning objective, interface, algorithm, data model, or edge-case reasoning. Renaming the same prompt/solution as “basic”, “boundary”, or another edition does not count.
- Run `pnpm audit:questions`; normalized prompts and reference solutions must not duplicate another visible problem, except an explicitly paired C/C++ exercise whose two implementations are different.
