...
const completions: Record<Language, Completion[]> = {
  ...
  GDB: [
    { label: 'break', type: 'keyword', detail: 'b → 設定中斷點', apply: 'break function_name', boost: 120 },
    { label: 'run', type: 'keyword', detail: 'r → 啟動程式', boost: 115 },
    { label: 'next', type: 'keyword', detail: 'n → 越過函式單步執行', boost: 110 },
    { label: 'step', type: 'keyword', detail: 's → 進入函式單步執行', boost: 110 },
    { label: 'continue', type: 'keyword', detail: 'c → 繼續執行' },
    { label: 'finish', type: 'keyword', detail: '執行到當前函式返回' },
    { label: 'print', type: 'keyword', detail: 'p → 查看運算式', apply: 'print variable_name' },
    { label: 'display', type: 'keyword', detail: '每次暫停自動顯示', apply: 'display variable_name' },
    { label: 'watch', type: 'keyword', detail: '監看寫入運算式', apply: 'watch variable_name' },
    { label: 'backtrace', type: 'keyword', detail: 'bt → 列出呼叫堆疊' },
    { label: 'frame', type: 'keyword', detail: '切換呼叫堆疊層', apply: 'frame 1' },
    { label: 'info locals', type: 'keyword', detail: '顯示區域變數' },
    { label: 'info args', type: 'keyword', detail: '顯示函式參數' },
    { label: 'info breakpoints', type: 'keyword', detail: '顯示中斷點' },
    { label: 'condition', type: 'keyword', detail: '設定條件式中斷點', apply: 'condition 1 expression' },
    { label: 'set variable', type: 'keyword', detail: '臨時修改變數', apply: 'set variable name = value' },
    { label: 'disassemble /m', type: 'keyword', detail: '混合原始碼反組譯', apply: 'disassemble /m function_name' },
  ],
};
