...
const gdbTargetTemplate = '#include <stdio.h>\n\nint main(void) {\n    int value;\n    scanf("%d", &value);\n    int result = value * 2;\n    printf("%d\\n", result);\n    return 0;\n}';
...
export function PlaygroundPanel() {
  ...
  const [targetSource, setTargetSource] = useState(gdbTargetTemplate);
  ...
  useEffect(() => {
    if (language !== 'GDB') return;
    const timer = window.setTimeout(() => localStorage.setItem('codedive-playground-GDB-target', targetSource), 450);
    return () => window.clearTimeout(timer);
  }, [language, targetSource]);
  ...
  const changeLanguage = (next: PlaygroundLanguage) => {
    ...
    if (next === 'GDB') setTargetSource(localStorage.getItem('codedive-playground-GDB-target') || playgroundExamples.GDB[0].targetSource || gdbTargetTemplate);
  ...
  const loadExample = (example: PlaygroundExample) => {
    ...
    if (example.targetSource) setTargetSource(example.targetSource);
  ...
  const reset = () => {
    ...
    setStdin('');
    if (language === 'GDB') setTargetSource(gdbTargetTemplate);
    setResult(null);
    setError('');
  };
  ...
  const run = async () => {
    ...
    try {
      const response = await fetch('/api/playground', {
        ...
        body: JSON.stringify({ language, source, stdin, targetSource: language === 'GDB' ? targetSource : undefined }),
      });
  ...
  const downloadSource = () => {
    const extension = language === 'C' ? 'c' : language === 'C++' ? 'cpp' : language === 'Python' ? 'py' : language === 'SQL' ? 'sql' : 'gdb';
    ...
    <section className="overflow-hidden rounded-2xl border bg-card shadow-sm">
      <div className="flex flex-col gap-3 border-b p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><h2 className="font-black">自由程式實驗室</h2><p className="text-sm text-muted-foreground">C、C++、Python、SQL 實作與 GDB 引導除錯都集中在這裡。</p></div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select id="playground-language" value={language} onChange={(event) => changeLanguage(event.target.value as PlaygroundLanguage)} className="h-10 rounded-lg border bg-background px-3 text-sm">
            {(['C', 'C++', 'Python', 'SQL', 'GDB'] as PlaygroundLanguage[]).map((item) => <option key={item}>{item}</option>)}
          </select>
      ...
      <div className="grid min-h-[560px] lg:grid-cols-[1.3fr_.7fr]">
        <div className="flex min-w-0 flex-col border-b bg-[#0d1321] lg:border-b-0 lg:border-r">
          <div className="flex items-center justify-between border-b border-white/10 bg-[#111827] px-4 py-3 text-slate-100"><strong className="text-sm">{language === 'GDB' ? 'session.gdb' : `main.${language === 'C' ? 'c' : language === 'C++' ? 'cpp' : language === 'Python' ? 'py' : 'sql'}`}</strong><Badge className="border-white/10 bg-white/10 text-slate-200">{language}</Badge></div>
          ...
          <div className="flex justify-end border-t border-white/10 bg-[#0a101c] p-3"><Button className="gap-2 bg-emerald-600 hover:bg-emerald-700" disabled={running} onClick={() => void run()}>{running ? <Loader2 className="animate-spin" /> : <Play />}{language === 'GDB' ? '執行 GDB 練習' : '執行程式'}</Button></div>
        </div>
        <div className="grid min-h-[420px] grid-rows-[auto_auto_1fr]">
          ...
          {language === 'GDB' && <div className="border-b p-4"><label className="mb-2 block text-sm font-black" htmlFor="gdb-target-source">目標 C 程式</label><p className="mb-2 text-xs leading-5 text-muted-foreground">先透過沙箱編譯目標程式，再逐條解析 GDB 批次指令；這是引導練習，不是連線到本機除錯器。</p><Textarea id="gdb-target-source" value={targetSource} onChange={(event) => { setTargetSource(event.target.value); setResult(null); setError(''); }} className="min-h-40 font-mono text-sm" maxLength={30000} /></div>}
          <div className="border-b p-4"><label className="mb-2 block text-sm font-black" htmlFor="playground-stdin">{language === 'GDB' ? '目標程式輸入 stdin' : '標準輸入 stdin'}</label><Textarea id="playground-stdin" value={stdin} onChange={(event) => setStdin(event.target.value)} className="min-h-28 font-mono text-sm" maxLength={10000} placeholder={language === 'Python' ? '例如：Ada' : '需要輸入時填在這裡；可留空'} /></div>
          <div className="flex min-h-0 flex-col bg-slate-950 text-slate-100">
            ...
            <div className="min-h-56 flex-1 p-4 font-mono text-sm leading-6">
              {running ? <span className="flex items-center gap-2 text-slate-400"><Loader2 className="size-4 animate-spin" />{language === 'GDB' ? '正在準備除錯情境…' : '正在編譯與執行…'}</span> : error ? <div className="flex gap-2 text-rose-300"><XCircle className="mt-0.5 size-4 shrink-0" /><span>{error}</span></div> : result ? <div><div className={`mb-3 flex items-center gap-2 text-xs font-bold ${result.status === 'Accepted' || result.status === 'GDB Session Ready' ? 'text-emerald-400' : 'text-amber-300'}`}>{result.status === 'Accepted' || result.status === 'GDB Session Ready' ? <CheckCircle2 className="size-4" /> : <XCircle className="size-4" />}{result.status}</div><pre className="whitespace-pre-wrap break-words">{result.error || result.stdout || '（程式沒有輸出）'}</pre></div> : <span className="text-slate-500">{language === 'GDB' ? '載入 DEMO 後執行，這裡會逐條說明除錯流程。' : '按「執行程式」後，結果會顯示在這裡。'}</span>}
            </div>
